...
Original file is located at
    https://colab.research.google.com/drive/1J_T_FlZCyubMCWSnKMF3xphwvFiYlQ2P
"""
...
def main(csv_path=None):
    ...
    # RF12 - Exportação e Persistência de Ficheiros I/O (CSV e JSON)
    df_processado.to_csv("vendas_tratadas.csv", index=False)
    df_processado.to_json("vendas_tratadas.json", orient="records", indent=4)
    clientes_seg.to_csv("clientes_segmentados.csv", index=False)
    print("\n[I/O Export] Arquivos 'vendas_tratadas.csv', 'clientes_segmentados.csv' e 'vendas_tratadas.json' criados com sucesso!")

    # Tratamento Multiplataforma para Download Automatizado (Google Colab vs Ambiente Local)
    if 'google.colab' in sys.modules:
        from google.colab import files
        print("\n[Ambiente Colab] A iniciar download dos ficheiros gerados...")
        files.download("vendas_tratadas.csv")
        files.download("vendas_tratadas.json")
        files.download("clientes_segmentados.csv")
        files.download("grafico_receita_regiao.png")
        files.download("grafico_top_produtos.png")
        files.download("grafico_distribuicao_receitas.png")
        files.download("grafico_boxplot_categorias.png")
    else:
        print("\n[Ambiente Local] Execução concluída. Todos os relatórios e imagens foram guardados na pasta de trabalho.")
...
if __name__ == "__main__":
    ...
    if not os.path.exists("vendas.csv"):
        ...
        df_base.to_csv("vendas.csv", index=False)
